...
import { ref, onMounted, computed, DefineComponent, defineExpose, VNodeRef } from "vue"
...
<template>
  <div ref="scroller" class="scroller h-10 overflow-y-scroll flex-auto">
    ...
    <template v-else>
      <component :is="props.component" v-for="item in items" :key="`${item[props.componentKey]}`" :item-id="item.id" />
    </template>
  </div>
</template>
