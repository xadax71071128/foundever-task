...
const routeIsHome = computed(() => router.currentRoute.value.name === ROUTE_CRYPTO_OVERVIEW.name)
const routeIsFavorites = computed(() => router.currentRoute.value.name === ROUTE_CRYPTO_FAVORITES.name)

const { t: print } = useI18n()

const cryptoStore = useCryptoStore()
const { cryptoList, cryptoFavorites } = toRefs(cryptoStore.state)
...
<template>
  <LayoutDashboard>
    <ViewCryptoList v-if="routeIsHome || routeIsFavorites" v-bind="viewProps" />
    <router-view v-else />
  </LayoutDashboard>
</template>
